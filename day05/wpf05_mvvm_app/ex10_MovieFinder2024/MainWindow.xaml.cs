...
namespace ex10_MovieFinder2024
{
    ...
    {
        // flag값
        private bool isFavorite = false; // 즐겨찾기인지 API로 검색한거지 false => openAPI, true => 즐겨찾기 보기
        ...
        private async void SerchMovie(string movieName)
        {
            ...
            var jsonResult = JObject.Parse(result);     // type.Parse(string 타입 변수)를 type으로 변경해줌.
            var total = Int32.Parse(jsonResult["total_results"].ToString());
            //await this.ShowMessageAsync("검색 수", total.ToString());
            var results = jsonResult["results"];
            var jsonArray = results as JArray;   // results가 json 배열이기 때문에 JArry는 리스트와 동일 -> foreach 사용가능
        ...
        {
            // 재겸색하면 데이터그리드 결과가 바뀌면서 이 이벤트가 다시 발생
            try
            {
